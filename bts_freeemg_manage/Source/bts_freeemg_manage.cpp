// bts_freeemg_manage.cpp : Define las funciones de la biblioteca estática.
//
...
namespace bts_manage_tools {
	...
	batch bts_bm_manager::CompleteDequeue(std::chrono::steady_clock::time_point initial_time) {
		batch measuredBatch;
		measuredBatch.takenMoment = std::chrono::high_resolution_clock::now();
		...
		for (channel_ptr = this->readable_channels.begin(); channel_ptr < this->readable_channels.end(); channel_ptr++) {
			...
			for (int sample_n = 0; sample_n < queueSize; sample_n++)
			{
				...
				sample samp{
					initial_time + std::chrono::milliseconds(Si),
					Si,
					value,
					exitStatus
				};
	...
	/*
	boost::json::object bts_bm_manager::CompleteDequeuejson(boost::chrono::steady_clock::time_point initial_time) {
		boost::json::object json_batch;
		json_batch["BatchTimeStamp"] = boost::chrono::to_string(boost::chrono::high_resolution_clock::now());

		int queueSize = this->MaxQueueSize();
		if (queueSize == -1) {
			json_batch["data"] = boost::json::array();
			return json_batch;
		}
		// measuredBatch.channelsData = map<int, vector<sample>>();
		vector<int>::iterator channel_ptr;
		boost::json::array channels_array;
		for (channel_ptr = this->readable_channels.begin(); channel_ptr < this->readable_channels.end(); channel_ptr++) {
			int ch_index = *channel_ptr;
			boost::json::object channel_batch;
			channel_batch["Channel"] = ch_index;
			...
			for (int sample_n = 0; sample_n < queueSize; sample_n++)
			{
				...
				boost::json::object sample_json;
				sample_json["TimeStamp"] = boost::chrono::to_string(initial_time + boost::chrono::milliseconds(Si));
				sample_json["index"] = Si;
				sample_json["value"] = value;
				sample_json["ExitStatus"] = exitStatus;
			...
			channel_batch["data"] = samples_array;
			channels_array.push_back(channel_batch);
		}
		json_batch["data"] = channels_array;
		return json_batch;
	}
