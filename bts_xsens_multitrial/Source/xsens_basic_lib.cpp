...
namespace xsens_manage_tools {
	...
	void xsensManager::ConfigurePatient() {
		...
		*/
		...
		this->instance->xme().setSampleRate(100);
	...
	void xsensManager::ReviewStatus() {
		...
		// std::printf("| Master Devices Connected: %1d                                                 |\n", (int)(masterStatus.empty() ? 0 : 1));
		std::printf("| Sampling frequencies allowed:                                                |\n");
		std::printf("===============================================================================\n");
		XsIntArray& freqs = this->instance->xme().determineSupportedUpdateRates(this->instance->xme().detectedDevices(), 0, false);
		XsIntArray::iterator freqs_it;
		for (freqs_it = freqs.begin(); freqs_it < freqs.end(); freqs_it++) {
			std::printf("| %d                                                |\n", *freqs_it);
		}
	}
